...
# -----------------------------
    ...
    TRAIN_DIR = _BASE / "train_ml_imputed"
    TEST_DIR  = _BASE / "test_ml_imputed"
    OOF_DIR   = _BASE / "yq_cls_out"   # 있을 때만 KPI/TEST 사용
    ...
    try:
        oof_files  = list(OOF_DIR.glob(f"oof_summary_{sel_line}_{product}*.csv"))
        test_files = list(OOF_DIR.glob(f"test_pred_{sel_line}_{product}*.csv"))
        if oof_files and test_files:
            oof_df  = pd.read_csv(oof_files[0])
            test_df = pd.read_csv(test_files[0])
            from sklearn.metrics import f1_score
            y_true = pd.to_numeric(oof_df.get("Y_Class"), errors="coerce")
            y_pred = pd.to_numeric(oof_df.get("y_direct_clf"), errors="coerce")
            macro    = f1_score(y_true, y_pred, average="macro",    zero_division=0)
            weighted = f1_score(y_true, y_pred, average="weighted", zero_division=0)
            micro    = f1_score(y_true, y_pred, average="micro",    zero_division=0)
            cnt = test_df.get("y_direct_clf", pd.Series(dtype=int)).value_counts().reindex([0,1,2], fill_value=0)
    ...
    try:
        ...
        if not train_fp or not train_fp.exists():
            ...
            y_min = float(np.nanmin(yq_train[valid_train])) if valid_train.any() else 0.0
            y_max = float(np.nanmax(yq_train[valid_train])) if valid_train.any() else 1.0
            ...
            st.markdown(" ")
            c1, c2 = st.columns([7, 1])
            with c1: st.markdown("### ⚙️ 이상치 탐지 모델 결과")
            with c2:
                ...
                sel_feature = sel_display.replace(" ⭐", "")  # ← 표시용 별 제거(접미사)
            ...
            if sel_feature in df_train.columns:
                ...
                fig_feat = go.Figure()
                fig_feat.add_trace(go.Scatter(x=xs_time, y=y_feat, mode="lines+markers",
